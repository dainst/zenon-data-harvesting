...
subject_table = open("records/subject_table_aegyptica.csv", "w")
...
def create_new_record(article_soup, out, category, url, volume):
    doi = None
    pdf = None
    pages = None
    review = False
    ...
    recent_record.add_field(Field(tag='007', indicators=None, subfields=None, data='cr  uuu      uuuuu'))
    ...
    recent_record.add_field(Field(tag='590', indicators=[' ', ' '], subfields=['a', '2019xhnxaegyp']))
    ...
    if review != True and "Fundchronik" not in title:
        search_subject(year, search_subject_title, search_subject_person, title, recent_record)
    recent_record.add_field(Field(tag='264', indicators=[' ', ' '],
                                          subfields=['a', 'Heidelberg', 'b', 'Propylaeum', 'c', year]))
    ...
    if year=='2018' and volume=='3':
        volume_description = "Aegyptiaca. Journal of the History of Reception of Ancient Egypt: Mnemohistory and Cultural Memory - Essays in Honour of Jan Assmann"
    else:
        volume_description = "Aegyptiaca. Journal of the History of Reception of Ancient Egypt"
    if volume != None:
        if category == "Besprechungen" or category == "Rezensionen / Reviews / Comptes rendus":
            recent_record.add_field(Field(tag='LKR', indicators=[' ', ' '],
                                          subfields=['a', 'ANA', 'b', '001527029', 'l', 'DAI01',
                                                     'm', print_title, 'n', '[Rez.in]: ' +volume_description+ ', ' +
                                                     volume + ' (' + year + ')']))
        else:
            recent_record.add_field(Field(tag='LKR', indicators=[' ', ' '],
                                          subfields=['a', 'ANA', 'b', '001527029', 'l', 'DAI01',
                                                     'm', print_title, 'n', volume_description + ', ' +
                                                     volume + ' (' + year + ')']))
    else:
        if category == "Besprechungen" or category == "Rezensionen / Reviews / Comptes rendus":
            recent_record.add_field(Field(tag='LKR', indicators=[' ', ' '],
                                          subfields=['a', 'ANA', 'b', '001527029', 'l', 'DAI01',
                                                     'm', print_title, 'n', '[Rez.in]: ' +volume_description+ ', ' + year]))
        else:
            recent_record.add_field(Field(tag='LKR', indicators=[' ', ' '],
                                          subfields=['a', 'ANA', 'b', '001527029', 'l', 'DAI01',
                                                     'm', print_title, 'n', volume_description +
...
for page in range(1, 8):
    ...
    for list_element in list_elements:
        ...
        if issue_information.split("(")[1].split(")")[0] not in issues:
            issues.append(issue_information.split("(")[1].split(")")[0])
            out = open('records/aegyptiaca/aegyptiaca' + list_element.text.split("(")[1].split(")")[0] + '.mrc', 'wb')
        else:
            out = out
        volume = issue_information.split("Nr. ")[1].split(" (")[0]
        print(volume)
        print(issues)
        url = list_element.find('a', class_='title')['href']
        user_agent = 'Mozilla/5.0 (X11; Linux x86_64; rv:66.0)'
        values = {'name': 'Helena Nebel',
                  'location': 'Berlin',
                  'language': 'Python'}
        ...
        for article in issue_soup.find_all('div', class_='obj_article_summary'):
            ...
            if not any(word in article.text for word in
                ...
                if category in ["Sonstiges", "Literatur", "Editorial"]:
                    continue
                create_new_record(article_soup, out, category, url, volume)
                record_nr += 1
subject_table.close()
