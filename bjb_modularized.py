...
try:
    ...
    while not empty_page:
        ...
        for list_element in list_elements:
            ...
            current_item = int(year + str(max([int(vol) for vol in re.findall(r'\d+', issue_information)])).zfill(3))
            if current_item > last_item_harvested_in_last_session:
                ...
                if "Beilage" not in volume_title:
                    volume = volume_title.split(":")[0].replace("Bd. ", "").strip().replace(")", "").split(" (")[0]
                else:
                    continue
                volume_name = volume_title.split(": ")[1].split("|")[0].strip()
                if len(re.findall(r'\d{4}', volume_title)) > 0:
                    volume_year = min([int(year) for year in re.findall(r'\d{4}', volume_title)])
                else:
                ...
                for article in issue_soup.find_all('div', class_='obj_article_summary'):
                    ...
                    if category not in ['Titel', 'Inhalt', 'Verbesserungen', 'Vorwort/Widmung', 'Abkürzungen']:
                        ...
                        for key in years_published_in:
                            if publication_dict['publication_year'] >= key:
                                place_of_publication = publishers[key][0]
                                publisher = publishers[key][1]
                                place_of_production = producers[str(publication_dict['publication_year'])][0]
                                production = producers[str(publication_dict['publication_year'])][1]
                                break
                        ...
                        if category in ['Bildbeilage', 'Register', 'Miszellen', 'Chronik', 'Vereinsangelegenheiten_Statuten', 'Berichte', 'Jahresberichte']:
                            publication_dict['title_dict']['main_title'] = \
                                article_soup.find('meta', attrs={'name': 'citation_title'})['content'].replace("...", "") + ' ' \
                                + volume_name + ', ' + volume + ' (' + publication_dict['publication_year'] + ')'
                        else:
                        ...
                        publication_dict['authors_list'] = [HumanName(author['content']).last + ', ' + HumanName(author['content']).first
                                                            for author in article_soup.find_all('meta', attrs={'name': 'citation_author'}) if author != "Die Redaktion"]
