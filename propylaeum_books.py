...
def create_publication_dicts(last_item_harvested_in_last_session, *other):
    ...
    items_harvested = []
    try:
        url = 'https://books.ub.uni-heidelberg.de/propylaeum/catalog/series/archuge'
        ...
        if book_tags:
            ...
            for link in links:
                ...
                isbn_pdf_list = [re.findall(r'\d{3}-.*\d', tag.text)[0] for tag in ebook_soup.find_all('div', style="margin-top: 0px;")]
                isbn_pdf = isbn_pdf_list[0] if isbn_pdf_list else None
                isbn_list = [re.findall(r'\d{3}-.*\d', tag.text)[0] for tag in ebook_soup.find_all('p', style="margin-top: 1.2em")]
                ...
                if current_item > last_item_harvested_in_last_session:
                    ...
                    if isbn and isbn_pdf:
                        url = u'http://swb.bsz-bw.de/sru/DB=2.1/username=/password=/?query=pica.isb+%3D+"' + isbn + '"&version=1.1&operation=searchRetrieve&stylesheet=http%3A%2F%2Fswb.bsz-bw.de%2Fsru%2FDB%3D2.1%2F%3Fxsl%3DsearchRetrieveResponse&recordSchema=marc21&maximumRecords=1&startRecord=1&recordPacking=&sortKeys=none&x-info-5-mg-requestGroupings=none'
                        req = urllib.request.Request(url)
                        with urllib.request.urlopen(req) as response:
                        ...
                        if identifiers_627:
                            ...
                            for record in new_reader:
                                ...
                                publication_dict['fields_590'] = [] # Sabine fragen!
